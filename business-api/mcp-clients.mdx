...
Use any official [Model Context Protocol](https://modelcontextprotocol.io/) client that supports **Streamable HTTP**. Pass the MCP URL and send `Authorization: Bearer` with your D3 API key on every HTTP request (SDKs differ in how they expose headers or token providers).
