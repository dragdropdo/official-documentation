...
Your webhook endpoint must accept **POST** requests with a JSON body. D3 sends the event payload along with an `X-D3-Signature` header (when a `webhook_secret` is configured). Your server should:
...
app.post(
  ...
  (req, res) => {
    const signature = req.headers["x-d3-signature"] as string;
    const body = req.body;
...
def handle_webhook():
    signature = request.headers.get("X-D3-Signature", "")
...
$signature = $_SERVER['HTTP_X_D3_SIGNATURE'] ?? '';
...
post '/webhooks/d3' do
  ...
  signature = request.env['HTTP_X_D3_SIGNATURE'] || ''
...
func handleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	signature := r.Header.Get("X-D3-Signature")
	if !verifySignature(body, signature, webhookSecret) {
...
public class D3WebhookServer {
    ...
    private static void handleWebhook(HttpExchange exchange) throws Exception {
        ...
        String signature = exchange.getRequestHeaders().getFirst("X-D3-Signature");
...
When a `webhook_secret` is configured, every webhook request includes an `X-D3-Signature` header containing the HMAC-SHA256 hex digest of the raw request body, keyed with your secret.
...
def verify_d3_signature(raw_body, signature_header, secret)
...
func verifyD3Signature(rawBody []byte, signatureHeader, secret string) bool {
	mac := hmac.New(sha256.New, []byte(secret))
	mac.Write(rawBody)
	expected := hex.EncodeToString(mac.Sum(nil))
...
public static boolean verifyD3Signature(
    byte[] rawBody,
    String signatureHeader,
    String secret
) throws Exception {
...
- **Verify signatures** – always validate the `X-D3-Signature` header using your `webhook_secret` and a constant-time comparison.
